package com.example.testmediacodec.encode
 ...
 * @description:
 */
class EncodeUsingSurfaceActivity  : Activity() {
    ...
    private fun encodeUsingSurfaceAsync() {
      ...
      //  Log.d(TAG, "codec info: ${MediaClassJsonUtils.toJson(encoder.codecInfo).toString()}")
        Log.d(TAG, "codec info: encoder.codecInfo.toString()}")
        ...
        val bitmap = BitmapFactory.decodeResource(resources, R.raw.test_img_1280, options)
        while (!outputEnd.get())
        {
